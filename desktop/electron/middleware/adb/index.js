...
function parsePackagePath(rawText = '') {
  return rawText
    .split(/\r?\n/)
...
async function extractApkIcon(apkPath, packageName) {
  const iconPath = path.join(path.dirname(apkPath), `${sanitizeFileName(packageName)}.icon`)
  const script = `
Add-Type -AssemblyName System.IO.Compression.FileSystem
$apkPath = [System.IO.Path]::GetFullPath('${apkPath.replaceAll('\'', '\'\'')}')
$iconPath = [System.IO.Path]::GetFullPath('${iconPath.replaceAll('\'', '\'\'')}')
$zip = [System.IO.Compression.ZipFile]::OpenRead($apkPath)
try {
  $entry = $zip.Entries |
    Where-Object {
      $_.FullName -match '\\.(png|webp)$' -and
      $_.FullName -match '(?i)(mipmap|drawable)' -and
      $_.FullName -match '(?i)(ic_launcher|app_icon|icon)' -and
      $_.FullName -notmatch '(?i)(background|foreground|monochrome|round)'
    } |
  ...
  if (-not $entry) {
    $entry = $zip.Entries |
      Where-Object {
        $_.FullName -match '\\.(png|webp)$' -and
        $_.FullName -match '(?i)(ic_launcher|app_icon|icon)'
      } |
      Sort-Object Length -Descending |
      Select-Object -First 1
  }
  ...
  if (-not $entry) {
    throw 'No bitmap app icon found in APK'
  }

  [System.IO.Compression.ZipFileExtensions]::ExtractToFile($entry, $iconPath, $true)
...
`
  ...
  const { stdout } = await sheller([powershell, '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', script], {
    encoding: 'utf8',
  })
...
async function getAppIcon(deviceId, packageName) {
  if (!deviceId || !packageName) {
    return ''
  }

  const packagePath = parsePackagePath(await deviceShell(deviceId, `pm path ${packageName}`))

  if (!packagePath) {
    return ''
  }

  const cacheDir = path.join(os.tmpdir(), 'escrcpy-vr-app-icons', sanitizeFileName(packageName))
  fs.mkdirSync(cacheDir, { recursive: true })

  const apkPath = await pull(deviceId, packagePath, { savePath: cacheDir })
  const iconPath = await extractApkIcon(apkPath, packageName)
